...
def create_response(data=None, success=True, paginator=None, error_code=ResponseCodes.success):
    response = {'success': success, 'error_code': int(
        error_code.value), 'error_message': get_error_message(error_code)}
    if paginator is not None:
        ...
        response['pagination']['next'] = paginator.get_next_link()
        response['pagination']['previous'] = paginator.get_previous_link()
...
def get_error_message(error_code):
    ...
    if error_code == ResponseCodes.invalid_credentials:
        ...
        return 'You didn\'t pass reCAPTCHA challenge! You\'ll need to signin again!'
